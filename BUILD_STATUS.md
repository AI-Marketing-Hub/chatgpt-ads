...
Linux is the current guarded-workflow execution target. Native Windows guarded workflows are unavailable in this release: the new doctor reports the unsupported capability explicitly rather than silently weakening path protection. See [Windows boundary](docs/WINDOWS.md). Hosted native Windows restriction checks and doctor passed on Python 3.11 and 3.14 for v0.3.0. They do not establish guarded Windows workflow support. macOS remains unverified.

Earlier v0.2.1 Codex and Claude keyboard localhost browser simulations remain historical evidence for those exact inputs. They do not validate the changed v0.3.0 implementation. Fresh v0.3.0 advisory runs are synthetic; the [probe assessment](acceptance/v030/probe-assessment.json) records successes and remaining errors.
...
v0.3.1 addresses manifest metadata screening, exclusive guarded archive output, regression coverage and dependency verification. Current CI results are linked from the README badge. Dated acceptance/v030 records describe their original evidence scope. The supplied cover and workflow image are illustrative artwork, not an observed account screenshot.
