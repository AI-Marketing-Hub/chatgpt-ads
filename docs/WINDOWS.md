...
Before running the portable analyzer, create a local virtual environment and install `requirements/validation.txt` with `python -m pip install --require-hashes -r requirements/validation.txt`. This includes `tzdata`, since Windows does not supply the IANA timezone database used by Python's `zoneinfo`. See [Python timezone data sources](https://docs.python.org/3/library/zoneinfo.html#data-sources). No report timezone is substituted or guessed.
